...
#include <string.h>
#include <sys/mman.h>
#include "assembly_code_generation.h"
#include "symbol_t.h"
...
void jit_compile_expression_to_arm(
        ...
        void *out_buffer) {
    assembly_code_generation(expression, externs, out_buffer);
}
...
init_symbols() {
    ...
    symbols[offset].pointer = (void *) (&my_div);
    ++offset;

    symbols[offset].name = func_names[offset];
    symbols[offset].pointer = (void *) (&my_mod);
    ++offset;

    symbols[offset].name = func_names[offset];
    symbols[offset].pointer = (void *) (&my_inc);
    ++offset;

    symbols[offset].name = func_names[offset];
    symbols[offset].pointer = (void *) (&my_dec);
    ++offset;
...
call_function_and_print_result(void *addr) {
    ...
    jited_function_t function = (int (*)()) (addr);
    int result = function();
    printf("%d\n", result);
}
